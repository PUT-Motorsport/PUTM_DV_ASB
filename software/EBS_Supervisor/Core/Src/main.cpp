...
StopWerker stop_werker(&werker);
ErrorWerker err_werker(&werker);
ContinuousMonitoringWerker monitoring_werker(&werker, &err_werker, &stop_werker);
InitialCheckUpWerker init_werker(&werker, &err_werker, &monitoring_werker);

#if USE_TEST
TestWerker test_werker1(&werker);
TestWerker test_werker2(&werker, &test_werker1);
#endif
...
int main(void)
{
  ...
  /* USER CODE BEGIN 2 */
	...
	test_werker1.fun = &test_fun1;
	test_werker2.fun = &test_fun2;
#else
	werker = &init_werker;
#endif
...
void test_fun1()
{
	...
	while(true)
	{
		if(led_timer.checkIfTimedOutThenReset())
		{
			led_ok.toggle();
		}
	}
...
void test_fun2()
{
	Timer led_timer(1000);
	Timer timer(2000);
	while(true)
	{
		if (led_timer.checkIfTimedOutThenReset()) led_ok.activate();
		if (timer.checkIfTimedOutThenReset())
		{
			werker = &test_werker1;
			return;
		}
