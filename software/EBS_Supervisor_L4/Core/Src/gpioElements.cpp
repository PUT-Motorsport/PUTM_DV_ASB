...
GpioElement::GpioElement(const GPIO_TypeDef *const port, const uint32_t pin,
...
void GpioOutElement::activate() {
    HAL_GPIO_WritePin((GPIO_TypeDef *)(this->port), this->pin,
                      (this->is_inverted ? GPIO_PIN_RESET : GPIO_PIN_SET));
}

void GpioOutElement::deactivate() {
    HAL_GPIO_WritePin((GPIO_TypeDef *)(this->port), this->pin,
                      (this->is_inverted ? GPIO_PIN_SET : GPIO_PIN_RESET));
}
...
bool GpioInElement::isActive() {
     ...
     *
     */
    return ((HAL_GPIO_ReadPin((GPIO_TypeDef *)(this->port), this->pin) ==
             GPIO_PIN_SET) != this->is_inverted);
}
